	...
	class Note {
		...
		public function submitNote($body, $user_to) {
			...
			if($check_empty != "") {
				...
				//insert note
				$query = mysqli_query($this->con, "INSERT INTO notes (id,body,added_by,user_to,date_added,user_closed,deleted,likes) VALUES (NULL, '$body', '$added_by', '$user_to', '$date_added', 'no', 'no', '0')");
				...
				if($user_to != 'none') {
					...
					$notification->insertNotification($returned_id, $user_to, "profile_note");
		...
		public function loadNotesFriends($data, $limit) {
			...
			$data_query = mysqli_query($this->con, "SELECT * FROM notes WHERE deleted='no' ORDER BY date_added DESC");

			if(mysqli_num_rows($data_query) > 0) {


				$num_iterations = 0; //Number of results checked (not necasserily notd)
				$count = 1;
				...
				while($row = mysqli_fetch_array($data_query)) {
					...
					//Prepare user_to string so it can be included even if not noted to a user
					if($row['user_to'] == "none") {
						$user_to = "";
					}
					else {
					...
					//Check if user who noted, has their account closed
				    ...
				    <script>
						...
						$str .= "<div class='status_note' onClick='javascript:toggle$id()'>
									<div class='note_profile_pic'>
										<img src='$profile_pic'>
									</div>
									...
									<div class='newsfeedNoteOptions'>
										<br><br>
										Comments ($comments_check_num)&nbsp;
										<iframe src='like_note.php?note_id=$id' scrolling='no'></iframe>
									</div>

								</div>
								<div class='note_comment' id='toggleComment$id' style='display:none;'>
									<iframe src='comment_frame_note.php?note_id=$id' id='comment_iframe' frameborder='0'></iframe>
								</div>
								<hr>";
								...
								<script>
									...
									$(document).ready(function() {
										...
										$('#note<?php echo $id; ?>').on('click', function(result){

											bootbox.confirm("Are you sure want to delete this note?", function(result) {

												$.note("_includes/form_handlers/delete_note.php?note_id=<?php echo $id; ?>", {result:result});
				...
				if($count > $limit)
					...
					$str .= "<input type='hidden' class='noMoreNotes' value='true'><p style='text-align: center;'> No more notes to show! </p>";
			}
		...
		public function loadProfileNotes($data, $limit) {
			...
			$str = ""; //String to return
			$data_query = mysqli_query($this->con, "SELECT * FROM notes WHERE deleted='no' AND ((added_by = '$profileUsername' AND user_to = 'none') OR user_to = '$profileUsername') ORDER BY id DESC");
		...
		public function getSingleNote($note_id) {
			...
			$str = ""; // String to return
			$data_query = mysqli_query($this->con, "SELECT * FROM notes WHERE deleted='no' AND id = '$note_id' LIMIT 1");
				...
				if($user_logged_obj->isFriend($added_by)) {
				    ...
				    <script> // This doesn't work in Firefox
						...
						<?php
							...
							$str .= "
												<div class='status_note' onClick='javascript:toggle$id()'>
